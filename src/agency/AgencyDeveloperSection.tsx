...
export default function AgencyDeveloperSection() {
  ...
  return (
    <section
      ...
      className="py-24 lg:py-32 bg-[#06080d] relative"
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1280px] px-6 md:px-8 lg:px-12">
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-br from-white/[0.03] to-white/[0.01] p-8 lg:p-10">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
            <div>
              <div className="inline-flex items-center gap-1 p-1 rounded-xl bg-white/[0.04] border border-white/[0.06] mb-8 max-w-full overflow-x-auto">
                {tabs.map((t, i) => (
                  <button key={t.id} onClick={() => setActive(i)}
                    className={`relative px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1.5 sm:gap-2 overflow-hidden shrink-0 whitespace-nowrap ${active === i ? "bg-white/[0.1] text-white shadow-sm" : "text-white/40 hover:text-white/70"}`}>
                    {t.icon}{t.label}
                  </button>
                ))}
              </div>
              ...
              <div className="relative" style={{ minHeight: "220px" }}>
                {tabs.map((t, i) => (
                  <div key={t.id} className={`${active === i ? "relative opacity-100" : "absolute inset-0 opacity-0 pointer-events-none"} transition-opacity duration-500`}>
                    <h3 className={`text-2xl lg:text-[1.7rem] font-bold text-white leading-snug mb-6 ${active === i ? "dev-content-enter" : ""}`} style={{ fontFamily: "var(--font-display, 'Inter', sans-serif)" }}>{t.headline}</h3>
                    <ul className="space-y-4">
                      {t.bullets.map((b, bi) => (
                        <li key={bi} className={`flex items-start gap-3 ${active === i ? "dev-bullet-in" : ""}`} style={active === i ? { animationDelay: `${150 + bi * 80}ms` } : undefined}>
                          ...
                          <span className="text-[#b8bfcc] text-[15px] leading-relaxed">{b}</span>
                        </li>
                      ))}
                    </ul>
                    <div className={`mt-8 ${active === i ? "dev-bullet-in" : ""}`} style={active === i ? { animationDelay: `${150 + t.bullets.length * 80 + 40}ms` } : undefined}>
                      <a href="#" className="inline-flex items-center h-[46px] px-7 rounded-full bg-gradient-to-r from-cyan-500 to-violet-500 text-white text-sm font-semibold hover:scale-[1.02] transition-all duration-300">Explore Platform</a>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="relative" style={{ minHeight: "320px" }}>
              <div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-56 h-56 rounded-full blur-[100px] transition-colors duration-700 ${tab.glowColor}`} />
              {mockups.map((Mockup, i) => (
                <div key={i} className={`absolute inset-0 flex items-center justify-center dev-mockup-panel ${active === i ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-[0.97] translate-y-2 pointer-events-none"}`} aria-hidden={active !== i}>
